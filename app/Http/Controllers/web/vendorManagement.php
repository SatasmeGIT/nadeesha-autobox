...
{
    public function gmailCallBackNew(){
      return Socialite::driver('google')->redirect();
      
    }
    
     public function handleCallbackGmail(){
     $user  =  Socialite::driver('google')->user();
     $data = DB::table('users')->where('email',$user->email)->first();
     
     
     if($data){
          session(['vendor_data' => $data]);
          return redirect()->route('web.dashboardIndex'); 
         
     }else{
         return redirect()->route('web.vendor.register');
     }
    ...
    public function login(Request $request)
    {
        ...
        if (Auth::attempt($credentials)) {
            ...
            if($request->origin == "my_inquiry"){
                return response()->json(['code' => 'my_inquiry', 'msg' => "Login Success"]);  // if it has no package activated need to refirect guest and hide button
  
            }
            
            //redirect if comes from inquery
            if($request->origin == "inquery"){
                
             $check = DB::table('assign_packages')->where('customer_id',$vendor_details->id)->wherenot('package_id',5)->where('package_expire_date', '>', Carbon::now())->first(); // check has package or not and not a free package
             if($check){
                return response()->json(['code' => 'visible_inquery', 'msg' => "Login Success"]);  // it package has redirect inquery ads
             }else{
                return response()->json(['code' => 'loged_but_no_package', 'msg' => "Login Success"]);  // if it has no package activated need to refirect guest and hide button
             }  
    ...
    public function index($id = null)
    {
        ...
        return view('Web.vendorLogin',compact('id'));
    }
