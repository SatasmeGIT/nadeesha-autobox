...
class CreateAdController extends Controller
{
    public function index()
    {
        ...
        $ad_data = DB::table('assign_packages')->where('customer_id',session('vendor_data')->id)->first(); // get customer ad count 
        ...
        $vehicle_types = DB::table('vehicle_types')->where('vt_status', 1)->orderBy('vt_name', 'asc')->pluck('vt_name', 'id');
        return view('Web.create_ads', compact('district', 'vehicle_types', 'package_max_image','ad_data'));
    }

    public function getBrands(Request $request)
    {
        $data = DB::table('brand')->where('vt_id', $request->value)->where('status', 1)->orderBy('brand_name', 'asc')->get();
        return $data;
    }

    public function getModels(Request $request)
    {
        $data = DB::table('model')->where('brand_id', $request->value)->where('status', 1)->orderBy('model_name', 'asc')->get();
        return $data;
    }
    ...
    public function create_ad(Request $request)
    {
        ...
        $request->validate([
            ...
            'price' => 'numeric|nullable', //remove price cuase client ask so 
            'vehicle_type' => 'required',
            'brand_id' => 'required',
            'model_id' => 'required',
            
            'phone' => 'required|digits:9',
            'secondary' => 'nullable|digits:9',
            // 'condition' => 'required',
          ...
          $package_expired_check = DB::table('assign_packages')
        ->where('customer_id', session()->get('vendor_data')->id)
        ->whereDate('package_expire_date', '<', now()) // check package expired or not
        ->first();
        
        if($package_expired_check){
             // needs to show error message
            return response()->json(['code' => 'false', 'msg' => "Your package has expired."]); 
        }
        ...
        try {
            //get package expire date
             $package_expired_check = DB::table('assign_packages')
              ->where('customer_id', session()->get('vendor_data')->id)
              ->pluck('package_expire_date')->first();
            ...
            $ads_id = DB::table('ads')->insertGetId([
                ...
                'ad_price' => $request->price ?? 0,
                'ad_view_count' => 0, //need to change
                'ad_expire_date' => $package_expired_check, //ads only visible when package is active
                'status' => 1,
                'adminStatus' => 1,
                'vehicle_types_id' => $request->vehicle_type,
                'brands_id' => $request->brand_id,
                'models_id' => $request->model_id,
                'ads_condition' => $request->condition ?? "Not specified",
                ...
                'contact_optional' => $request->secondary,
            ]);
    ...
    {
        $data = DB::table('assign_packages')->where('customer_id',session('vendor_data')->id)->first();

        return view('Web.ads_appreved',compact('data'));
    }
}
