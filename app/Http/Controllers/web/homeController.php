...
class homeController extends Controller
{
    public function index()
    {
        ...
        $banner = DB::table('banner')->get(); // banner
        $vehicle_types = DB::table('vehicle_types')
        ->where('vt_status', 1)
        ->orderBy('vt_name', 'asc') // Order by vt_name in ascending order
        ->pluck('vt_name', 'id');
       ...
       $latest_ads = DB::table('ads')
        ...
        ->take(6)
        ->select('ads_images.name', 'ads.ad_title', 'ads.ad_price', 'ads.ad_title', 'ads.id', 'vehicle_types.vt_name', 'ads.ad_district', 'ads.ad_city', 'ads.created_at', 'ads.ad_number', 'ads.is_top_id', 'ads.ad_view_count')
        ->get();

        return view('Web.index', compact(['latest_ads', 'districts', 'vehicle_types', 'slider','banner']));
    }
    ...
    public function detailed($id)
    {
        $today = now()->toDateString(); // Get today's date
           DB::table('ads')
        ->where('id', $id)
        ->increment('ad_view_count', 1); // ad count
        ...
        $detailed_ads = DB::table('ads')
            ...
            ->where('ads.id', $id)
            ->select('ads.*', 'ads_images.*', 'vehicle_types.vt_name', 'brand.brand_name', 'model.model_name', 'users.First_Name', 'users.Last_Name', 'users.email', 'users.Fb_link', 'users.Twitter_link', 'users.Linkedin_link', 'users.Youtube_link', 'users.district', 'users.city')
            ->get();
    ...
    ->where('vt_id', $id)
    ->where('status', 1)
    ->orderBy('brand_name', 'asc') // Order by vt_name in ascending order
    ->get();
    ...
    {
        $models = DB::table('model')->where('brand_id', $id)->where('status', 1)->orderBy('model_name', 'asc')->get();
        return $models;
    }
    ...
    public function showFilteredAds(Request $request)
    {
        ...
        return $query->where('vehicle_types_id', $type);
        }) //
        ->when($request->brand, function ($query, $brand) { // if brand and model not null those functions should be executed
         return $query->where('brands_id', $brand);
        })
        ->when($request->model, function ($query, $model) {
            return $query->where('models_id', $model);
        })
        ->leftJoin('ads_images', function ($join) {
            $join->on('ads.id', '=', 'ads_images.ads_id')
                ->whereRaw('ads_images.id = (SELECT id FROM ads_images WHERE ads_id = ads.id LIMIT 1)');
        ...
        ->orderBy('ads.created_at', 'desc')
        ->select('ads_images.name', 'ads.ad_title', 'ads.ad_price', 'ads.ad_title', 'ads.id', 'vehicle_types.vt_name', 'ads.created_at', 'ads.ad_number', 'ads.ad_city', 'ads.ad_district', 'ads.is_top_id', 'ads.ad_view_count')
        ->paginate(12);
